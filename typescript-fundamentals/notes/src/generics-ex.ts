...
function mapDict<T, V>(
  dict: Dict<T>,
  callback: (origValue: T, key: string) => V
): Dict<V> {
  var newDict: Dict<V> = {};
...
function filterDict<T, V>(
  dict: Dict<T>,
  predicate: (origValue: T) => boolean
): Dict<V> {
  var newDict: Dict<V> = {};
...
function reduceDict<T, V>(
  dict: Dict<T>,
  reducer: (accuValue: V, value2: T) => V,
  accumulator: V
): V {
  var result = accumulator;

  for (let key in dict) {
    result = reducer(result, dict[key]);
  }
